...
class PixelNeRFNet(torch.nn.Module):
    ...
    def encode(self, depths, images, poses, focal, z_bounds=None, c=None):
        """
        ...
        ############ feature extract #############
        self.encoder(depths.squeeze(0).squeeze(0), images) # latent torch.Size([3, 512, 150, 200]) 이미지(200*300) 넣어 512 ch. 150*200 size feature 뽑음
        # print(depths.squeeze(0).shape, depths_std.squeeze(0).shape, images.shape) # torch.Size([3, 1, 585, 428]) torch.Size([3, 1, 585, 428]) torch.Size([3, 3, 585, 428])
    ...
    def forward(self, xyz, coarse, viewdirs=None, far=False):
        # xyz: [1,192,3] (coarse True일 때는 3으로 나눈 값이 한 pixel에 해당 False일 때는 2로 나누기)
        # viewdirs: [1,192,8]
        ...
        with profiler.record_function("model_inference"):
            ...
            ########################### xyz points 에 대해 view space 변경 및 positional encoding #################################
            real_viewdirs = viewdirs # torch.Size([4800, 3])
                        
            # Transform query points into the camera spaces of the input views
            xyz = repeat_interleave(xyz, NS)  # (SB*NS, B, 3) # original 1 view: torch.Size([1, 4800 (eval_size), 3]) 3 view: torch.Size([3, 4800, 3])
            xyz_rot = torch.matmul(self.poses[:, None, :3, :3], xyz.unsqueeze(-1))[..., 0]
            xyz = xyz_rot + self.poses[:, None, :3, 3] # new 1 view: torch.Size([1, 4800 (eval_size), 3]) 3 view: torch.Size([3, 4800, 3])

            
            # * Encode the xyz coordinates
            if self.normalize_z: z_feature = xyz_rot.reshape(-1, 3)  # (SB*B, 3)
            else: z_feature = xyz.reshape(-1, 3)  # (SB*B, 3)

            # Positional encoding (no viewdirs)
            # if self.use_code and not self.use_code_viewdirs: z_feature = self.code(z_feature)
            ...
            z_feature = torch.cat((z_feature, viewdirs), dim=1) # (SB*B, )  torch.Size([192, 6]) # [64,6] [64,6] [64,6] 세 pixel에 해당하는 값으로 나누어짐

            # Positional encoding (with viewdirs)
            # if self.use_code and self.use_code_viewdirs: z_feature = self.code(z_feature)

            ############################## xyz points에 해당하는 feature space 뽑는 부분 #################################
            # feature space : torch.Size([1, 512, 150, 200]) 1장의 이미지(200*300) 넣어 512 ch. 150*200 size feature 뽑음
            ...
            # projected xyz 'uv: torch.Size([3, 4800, 2])
            ...
            ref_depth = self.encoder.index_depth(uv, None, self.image_shape)  # (SB * NS, latent, B)
            depth_feature = ref_depth.transpose(1, 2).reshape(-1, self.latent_size)  # (SB * NS * B, latent)

            mlp_input = torch.cat((latent, depth_feature, z_feature), dim=-1)

            ############################################# final input: torch.Size([192, 554]) ### latent : [4800*3,512] + z_featrue : [4800*3,42] ###
            ...
            ################################# mlp final input 넣어 rgb depth 구하는 부분 !!!!!!!!!!!! ###################################
            ...
            # print(self.mlp_fine.lin_out.weight.grad)
            # print(self.mlp_fine.lin_out_rgb.bias.grad)
