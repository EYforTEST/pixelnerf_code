...
seed_fix(100)
args, conf = util.args.parse_args(extra_args, training=True, default_ray_batch_size=32)
...
class PixelNeRFTrainer(trainlib.Trainer):
    ...
    def calc_losses(self, data, is_train, global_step=0):
        ...
        all_depth = data["depth"].to(device=device)  # (SB, NV, 1, H, W)
        ...
        for obj_idx in range(SB):
            ...
            depth = all_depth[obj_idx] # (NV, 1, H, W)
            ...
            depth_gt_all = (depth_0to1.permute(0, 2, 3, 1).contiguous().reshape(-1, 1))  # (NV, H, W, 1)
            rgb_gt_all = (images_0to1.permute(0, 2, 3, 1).contiguous().reshape(-1, 3)) # (NV, H, W, 3) 
            ...
            depth_gt = depth_gt_all[pix_inds]  # (ray_batch_size, 3)
        ...
        all_depth_gt = torch.stack(all_depth_gt)  # (SB, ray_batch_size, 3)
        ...
        src_depths = util.batched_index_select_nd(all_depth, image_ord) # (SB, NS, 1, H, W)
        ...
        # for fixed src views
        # src_depths = data["src_depth"].to(device=device) # (SB, NS, 1, H, W)
        ...
        print(f'rgb: {all_rgb_gt.mean().item()} {coarse.rgb.mean().item()} {fine.rgb.mean().item()} depth: {coarse.depth.mean().item()} {fine.depth.mean().item()}') 
    ...
    def vis_step(self, data, global_step, idx=None):
        ...
        depth = data["depth"][batch_idx].to(device=device) # torch.Size([19, 1, 589, 430])
        src_depth = data["src_depth"][batch_idx].to(device=device) # torch.Size([19, 1, 589, 430])
        ...
        source_views = src_images.permute(0, 2, 3, 1).cpu().numpy().reshape(-1, H, W, 3) # (NV, H, W, 3)
        ...
        with torch.no_grad():
            ...
            test_depth = src_depth

            net.encode(
                test_depth.unsqueeze(0), 
    ...
    def gen_video(self, data, global_step, idx=None):
        ...
        with torch.no_grad():
            ...
            for rays in tqdm.tqdm(torch.split(cam_rays.view(-1, 8), 2560, dim=0)):
