...
class Trainer:
    ...
    def __init__(self, net, train_dataset, test_dataset, args, conf, device=None):
        ...
        # self.lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=self.optim, gamma=0.95)
        # self.lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=self.optim, T_max=1000, eta_min=0)
    ...
    def gen_video(self, data, global_step):
        """
        Visualization step
        """
        return 
    ...
    def start(self):
        ...
        for epoch in range(start_epoch, self.num_epochs):
            if self.mode == 'gen':
                ...
                with torch.no_grad():
                    self.gen_video(test_data, global_step=step_id)
            ...
            for _ in range(self.num_epoch_repeats):
                for data in self.train_data_loader:
                    ...
                    if batch % self.print_interval == 0:
                        print("E", epoch, "B", batch, loss_str, " lr")
                    ...
                    ######################################### logging
                    if batch != 0 and batch % self.save_interval == 0 and (epoch > 0 or batch > 0):
                        print("saving")
                        if self.managed_weight_saving:
                    ...
                    if batch == 200: # and epoch % 2 == 0  
                        ...
                        with torch.no_grad():
                            vis, vis_vals, ssim_vals, ms_ssim_vals = self.vis_step(
                        ...
                        if vis is not None:
                            ...
                            imageio.imwrite(
                                os.path.join(
                                    self.visual_path,
                                    "{:04}_{:04}_psnr_{:.3f}_ssim_{:.3f}_msssim_{:.3f}).png".format(epoch, batch, vis_vals["psnr"], ssim_vals["ssim"], ms_ssim_vals["msssim"]),
                                ),
                                vis_u8,
                            )
