...
class NeRFRenderer(torch.nn.Module):
    ...
    def composite(self, model, rays, z_samp, coarse, sb=0):
        """
        rays: 128*8
        z_samp: 128*64
        ...
        with profiler.record_function("renderer_composite"):
            ...
            # (B, K, 3) # points = ray_origin + ray_dir * z_sample_point
            points = rays[:, None, :3] + z_samp.unsqueeze(2) * rays[:, None, 3:6] 
            points = points.reshape(-1, 3)  # (B*K, 3) train, eval: [128*64 (coarse) or 96 (fine), 3] vis_step: [300*400*64 = 78680000 or *96, 3]
            ...
            if sb > 0:
                points = points.reshape(
                    sb, -1, 3 
                )  # (SB, B'*K, 3) B' is real ray batch size [1, 128*64, 3]
            ...
            # eval_batch_size: 192 (64 와 96의 최소공배수) * X >> 4800 사용     
            ...
            if use_viewdirs:
                ...
                for pnts, dirs in zip(split_points, split_viewdirs):
                    ...
                    ######################## models.py PixelNeRFNet의 함수 forward로 이동 #########################
    ...
    def forward(
        ...
        with profiler.record_function("renderer_forward"):
            ...
            superbatch_size = rays.shape[0] # 이미지 갯수 1개

            rays = rays.reshape(-1, 8)  # (SB * B, 8) # rays: torch.Size([128, 8])
            
            z_coarse = self.sample_coarse(rays)  # (B, Kc) # z_coarse: torch.Size([128, 64]) # 하나의 ray에 sample 갯수는 64개
    ...
    @classmethod
    def from_conf(cls, conf, white_bkgd=False, lindisp=False, eval_batch_size=10240//10):
        return cls(
